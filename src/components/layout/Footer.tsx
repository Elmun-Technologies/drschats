        ...
        <div className="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-[1.2fr_1fr_1fr_1fr_1fr_1.1fr]">
          ...
          <div>
            ...
            <div className="mt-5 flex gap-3">
              <a href={BRAND.social.telegram} target="_blank" rel="noopener noreferrer" aria-label="Telegram" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-white/5 text-white/70 transition-all duration-300 hover:border-gold hover:text-gold hover:bg-gold/10 hover:scale-110 shadow-lg">
              ...
              <a href={BRAND.social.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-white/5 text-white/70 transition-all duration-300 hover:border-gold hover:text-gold hover:bg-gold/10 hover:scale-110 shadow-lg">
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
              ...
              <a href={BRAND.social.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-white/5 text-white/70 transition-all duration-300 hover:border-gold hover:text-gold hover:bg-gold/10 hover:scale-110 shadow-lg">
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
...
function FooterCol({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h2 className="mb-4 font-display text-sm font-extrabold uppercase tracking-widest text-gold">{title}</h2>
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <li>
      <Link href={href} className="inline-block text-sm text-white/70 transition-all duration-300 hover:text-gold hover:translate-x-1">
        {children}
      </Link>
    </li>
  );
